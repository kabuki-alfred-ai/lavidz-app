...
const SEGMENT_ID = 'd029b0db-8921-4b18-ac07-7f8480cedbe5'
...
export async function POST(req: Request) {
  try {
    ...
    // Step 1: Create contact with properties via direct fetch
    // (SDK routes audienceId to legacy endpoint that does not support `properties`)
    ...
    // Step 2: Add contact to waitlist segment
    const resend = new Resend(apiKey)
    await resend.contacts.segments.add({ contactId, segmentId: SEGMENT_ID })
